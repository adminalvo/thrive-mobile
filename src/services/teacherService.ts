...
export const teacherService = {
  async getTeacherGroups(teacherId: string): Promise<TeacherGroupItem[]> {
    try {
      ...
      // If teacher has no assigned groups yet, fetch recent groups for display
  ...
  async getTodaysClasses(teacherId: string): Promise<any[]> {
    try {
      const groups = await this.getTeacherGroups(teacherId);
      ...
      console.error('Error fetching today teacher classes:', e);
      return [];
    }
  },

  async getGroupRoster(groupId: string, date?: string): Promise<TeacherStudentRosterItem[]> {
    try {
      ...
      let students: any[] = (groupStudents || []).map((gs: any) => gs.students).filter(Boolean);

      // If no group_students are assigned, fetch active students as fallback roster
      ...
      // Fetch attendance if date specified
