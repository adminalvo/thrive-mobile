...
export const studentService = {
  ...
  async getStudentHomeData(studentId: string, forceRefresh = false): Promise<StudentHomeBatchData> {
    ...
    const result = await cacheManager.fetchWithCache<StudentHomeBatchData>(
      cacheKey,
      async () => {
        // 1. Fetch group IDs, student profile info & teacher directory in parallel
        const [groupRes, studentRes, teachersRes] = await Promise.all([
        ...
        let groupIds = Array.from(
          new Set((groupRes.data || []).map((d) => d.group_id).filter(Boolean))
        ) as string[];
        ...
        const schedules: LessonScheduleItem[] = (scheduleRes.data || []).map((s: any) => {
          ...
          const teacherName = teacherMap.get(group.teacher_id) || 'Təyin edilməyib';
  ...
  async getEnrolledGroupIds(studentId: string): Promise<string[]> {
    try {
      ...
      return Array.from(new Set(data.map((d) => d.group_id).filter(Boolean))) as string[];
    } catch (e) {
