...
export default function ParentDashboard() {
  ...
  const stats = [
    { title: "Övladlarım", value: children.length, icon: Users, color: "var(--aqua-teal)" },
    { title: "Qarşıdan Gələn Dərslər", value: upcomingClasses.length, icon: Calendar, color: "var(--ocean-blue)" }
  ];
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {upcomingClasses.length > 0 ? upcomingClasses.map(cls => (
                  <tr key={cls.id}>
                    <td className={styles.studentName}>{cls.studentName}</td>
                    <td className={styles.studentGroup}>{cls.group} - {cls.program}</td>
